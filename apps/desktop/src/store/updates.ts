...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `lliam-gov update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'lliam-gov update',
        command: result.command ?? 'lliam-gov update'
      })
    }
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'lliam-gov update'
