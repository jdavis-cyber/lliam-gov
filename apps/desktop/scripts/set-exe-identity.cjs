#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Lliam-GOV icon + version metadata onto the
// built Lliam-GOV.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Lliam-GOV.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `lliam-gov desktop`, the installer's
...
// path (which rebuilds via `lliam-gov desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Lliam-GOV.exe>
//
...
// Stamp the Lliam-GOV icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Lliam-GOV',
      FileDescription: 'Lliam-GOV',
      CompanyName: 'Jerome Davis',
      LegalCopyright: 'Copyright (c) 2026 Jerome Davis'
    }
  })

  console.log('[set-exe-identity] done — Lliam-GOV icon + identity stamped')
}
