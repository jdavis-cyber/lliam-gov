 ...
 *
 * Stamps the Lliam-GOV icon + identity onto the packed Windows Lliam-GOV.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `lliam-gov desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Lliam-GOV')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Lliam-GOV'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Lliam-GOV.exe keeps the stock Electron icon`)
  }
}
