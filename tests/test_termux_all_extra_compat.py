...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    """Lliam-GOV facelift: self-references are 'lliam-gov[X]' (was 'hermes-agent[X]')."""
    text = PYPROJECT.read_text()
    assert "termux-all = [" in text
    assert '"lliam-gov[termux]"' in text
    assert '"lliam-gov[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"lliam-gov[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
