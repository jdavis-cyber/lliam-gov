...
class TestGeminiShortRateLimitRetry:
    """Bounded auto-recovery for short 429 windows (Code Assist per-minute cap).

    Field note: live recovery is exercised by these unit tests (mocked HTTP),
    not by a real rate-limit event — a genuine 429 can't be triggered
    deterministically without burning quota.
    """

    @staticmethod
    def _err(status, retry_after):
        from agent.google_code_assist import CodeAssistError
        return CodeAssistError("x", status_code=status, retry_after=retry_after)

    def test_bounded_wait_retries_short_429(self):
        from agent.gemini_cloudcode_adapter import _bounded_rate_limit_wait
        # 23s hint → wait 24s (hint + 1s cushion), under the 30s cap.
        assert _bounded_rate_limit_wait(self._err(429, 23.0)) == 24.0

    def test_bounded_wait_skips_when_no_hint(self):
        from agent.gemini_cloudcode_adapter import _bounded_rate_limit_wait
        assert _bounded_rate_limit_wait(self._err(429, None)) is None

    def test_bounded_wait_skips_when_too_long(self):
        from agent.gemini_cloudcode_adapter import _bounded_rate_limit_wait
        # 120s hint exceeds the 30s cap → don't wait, fail fast.
        assert _bounded_rate_limit_wait(self._err(429, 120.0)) is None

    def test_bounded_wait_skips_non_429(self):
        from agent.gemini_cloudcode_adapter import _bounded_rate_limit_wait
        assert _bounded_rate_limit_wait(self._err(500, 5.0)) is None

    @staticmethod
    def _fake_429_with_hint():
        body = {
            "error": {
                "code": 429, "message": "rate", "status": "RESOURCE_EXHAUSTED",
                "details": [{
                    "@type": "type.googleapis.com/google.rpc.RetryInfo",
                    "retryDelay": "23s",
                }],
        ...
        return SimpleNamespace(status_code=429, text=json.dumps(body), headers={})

    def test_non_stream_waits_then_succeeds(self, monkeypatch):
        from agent import gemini_cloudcode_adapter as adp
        from agent.gemini_cloudcode_adapter import GeminiCloudCodeClient, ProjectContext

        client = GeminiCloudCodeClient()
        client._http = MagicMock()
        client._http.post.side_effect = [
            self._fake_429_with_hint(),
            SimpleNamespace(status_code=200, json=lambda: {"ok": True}),
        ]
        monkeypatch.setattr(adp.google_oauth, "get_valid_access_token", lambda: "tok")
        monkeypatch.setattr(
            client, "_ensure_project_context",
            lambda access_token, model: ProjectContext(project_id="p", tier_id="standard-tier"),
        )
        sentinel = object()
        monkeypatch.setattr(adp, "_translate_gemini_response", lambda payload, model: sentinel)
        slept = []
        monkeypatch.setattr(adp.time, "sleep", lambda s: slept.append(s))

        out = client._create_chat_completion(
    ...
    def test_non_stream_no_hint_raises_without_retry(self, monkeypatch):
        from agent import gemini_cloudcode_adapter as adp
        from agent.gemini_cloudcode_adapter import GeminiCloudCodeClient, ProjectContext
        from agent.google_code_assist import CodeAssistError
        ...
        client._http = MagicMock()
        client._http.post.side_effect = [
            SimpleNamespace(status_code=429, text=json.dumps(body), headers={}),
        ]
        monkeypatch.setattr(adp.google_oauth, "get_valid_access_token", lambda: "tok")
        monkeypatch.setattr(
            client, "_ensure_project_context",
