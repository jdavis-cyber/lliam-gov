# ============================================================================
# Lliam-GOV Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/jdavis-cyber/lliam-gov/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = "$env:LOCALAPPDATA\lliam-gov",
    [string]$InstallDir = "$env:LOCALAPPDATA\lliam-gov\lliam-gov",
...
$RepoUrlSsh = "git@github.com:jdavis-cyber/lliam-gov.git"
$RepoUrlHttps = "https://github.com/jdavis-cyber/lliam-gov.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Lliam-GOV Installer                      |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  Governed AI agent for secure desktop and CLI workflows. |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Install-Git {
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\lliam-gov\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\lliam-gov\git``
...
function Test-Node {
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\lliam-gov-node-extract"
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running lliam-gov processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/jdavis-cyber/lliam-gov/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/jdavis-cyber/lliam-gov/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/jdavis-cyber/lliam-gov/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\lliam-gov-$zipLabel.zip"
                $extractPath = "$env:TEMP\lliam-gov-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `lliam-gov.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install lliam-gov package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and lliam-gov.exe broken with
    ...
    # here, not 30 seconds later when the user runs `lliam-gov`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `lliam-gov dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `lliam-gov dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `lliam-gov dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up lliam-gov command..."
    ...
    # Add the venv Scripts dir to user PATH so lliam-gov is globally available
    # On Windows, the lliam-gov.exe in venv\Scripts\ has the venv Python baked in
    ...
    # Only needed on Windows where we install to %LOCALAPPDATA%\lliam-gov instead
    # of the Unix default ~/.lliam-gov
    ...
    Write-Success "lliam-gov command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.lliam-gov directory structure
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.lliam-gov/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.lliam-gov/.env"
        }
    } else {
        Write-Info "~/.lliam-gov/.env already exists, keeping it"
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.lliam-gov/config.yaml from template"
        }
    } else {
        Write-Info "~/.lliam-gov/config.yaml already exists, keeping it"
    }
...
"@
        ...
        Write-Success "Created ~/.lliam-gov/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.lliam-gov/"
    
    # Seed bundled skills into ~/.lliam-gov/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.lliam-gov/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.lliam-gov/skills/"
        } catch {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to ~/.lliam-gov/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'lliam-gov setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\lliam-gov-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\lliam-gov-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\lliam-gov-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.lliam-gov/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'lliam-gov setup'."
        return
    }
    ...
    # Run lliam-gov setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\lliam-gov.exe"
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'lliam-gov whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: lliam-gov gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: lliam-gov gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: lliam-gov gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HermesHome\lliam-gov\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   lliam-gov              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   lliam-gov setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   lliam-gov config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   lliam-gov config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   lliam-gov gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   lliam-gov update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/jdavis-cyber/lliam-gov/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
